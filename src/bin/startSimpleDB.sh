...
do
	...
	ssh $host "mkdir -p /tmp/phungb/simpledb/data;mkdir /tmp/phungb/simpledb/bin;mkdir /tmp/phungb/simpledb/lib;mkdir /tmp/phungb/simpledb/conf"
	rsync -a "$SIMPLEDB_ROOT/bin/" $host:/tmp/phungb/simpledb/bin
	rsync -a "$SIMPLEDB_ROOT/lib/" $host:/tmp/phungb/simpledb/lib
	rsync -a "$SIMPLEDB_ROOT/conf/" $host:/tmp/phungb/simpledb/conf
	echo "Done"
done
...
do
	...
	ssh $host "mkdir -p /tmp/phungb/simpledb/data/$port"
	rsync -a "$SIMPLEDB_ROOT/data/${host}_${port}/" $host:/tmp/phungb/simpledb/data/$port
	echo "Done"
done
...
do
	...
	then
		if ! xEnabled
		then
		    (exec ssh $host "cd /tmp/phungb/simpledb; java -classpath \
		    ...
		    "bash -c \"ssh $host \\\"cd /tmp/phungb/simpledb; java -classpath \
		...
		osascript -e "tell app \"Terminal\"
			do script \"echo -e \\\"\\\\033]0;Worker: $host:$port. Do not close this window when SimpleDB is running.\\\\007\\\"; ssh $host \\\"cd /tmp/phungb/simpledb; java -classpath \\\\\\\"bin/src:lib/*\\\\\\\" simpledb.parallel.Worker ${host}:$port $serverAddr \\\" \"
		end tell"
	fi
