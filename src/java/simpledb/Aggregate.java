...
public class Aggregate extends Operator {
  ...
  if (gfield != -1) { gfieldType = child_td.getFieldType(gfield); };

  if (afieldType == Type.INT_TYPE) {
      aggr = new IntegerAggregator(gfield, gfieldType, afield, aop);
  } else {
      aggr = new StringAggregator(gfield, gfieldType, afield, aop);
  ...
  aggrIter.rewind();
	...
	// some code goes here
  String name = aop.toString() + " " + this.aggregateFieldName();
  if (gfield == -1) {
      return new TupleDesc(new Type[] {Type.INT_TYPE}, new String[] {name});
  } else {
      return new TupleDesc(new Type[] {child_td.getFieldType(gfield),
              Type.INT_TYPE}, new String[] {this.groupFieldName(), name});
  }
    }
