    ...
    public void rollback(Long tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                Long current = tidToFirstLogRecord.get(tid);
                ...
                */
                ...
                /* begin/abort/commit record conists of
                ...
                */
                ...
                /* checkpoint record conists of

                   (int) record type
                   (long) transaction id
                   (int) # of transactions
                   (2x long) for each transation
                   (long) start offset
                */

                // keep track of undid pages
                HashSet<PageId> undid = new HashSet<PageId>();

                while (current < eof) {
                    int recordType = raf.readInt();
                    long tranId = raf.readLong();
                    if (recordType == COMMIT_RECORD) {
                        int size = raf.readInt();
                        raf.seek(raf.getFilePointer() + 2 * LONG_SIZE * size);
                        ...
                        // undo a page only once
                        // revert page to the information stored in its first update
                        if (!undid.contains(before.getId()) && tranId == tid) {
                            undid.add(before.getId());
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // check for checkpoint at begnning of file
                long current = raf.readLong();
                if (current > 0) {
                    raf.seek(current);
                    int recordType = raf.readInt();
                    Long noTid = raf.readLong();
                    int size = raf.readInt();
                ...
                while (current < raf.length()) {
                    ...
                    if (recordType == BEGIN_RECORD) {
                        ...
                        // add to losers
                        losers.add(tid);
                        tidToFirstLogRecord.put(tid, start);
                    } else if (recordType == ABORT_RECORD) {
                        // transaction aborted so we need to undo all the updates
                        losers.remove(tid);
                        this.rollback(tid);
                    ...
                    // read past offset no next thing
                    raf.seek(raf.getFilePointer() + LONG_SIZE);
                    current = raf.getFilePointer();
                }
