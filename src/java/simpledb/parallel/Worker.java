...
public class Worker {
    ...
    public class WorkingThread extends Thread {
        public void run() {
            while (true) {
                ...
                if (query != null) {
                    try {
                        CollectProducer col = (CollectProducer) query;
                        col.open();
                        while (col.hasNext()) {
                            col.next();
                        }
    ...
    public void localizeQueryPlan(OpIterator queryPlan) {
        ...
        if (queryPlan instanceof SeqScan) {
            SeqScan seq = (SeqScan) queryPlan;
            String alias = seq.getAlias();
            int tableId = Database.getCatalog().getTableId(seq.getTableName());
            seq.reset(tableId, alias);

        } else if (queryPlan instanceof Producer) {
            Producer producer = (Producer) queryPlan;
            producer.setThisWorker(this);

        } else if (queryPlan instanceof Consumer) {
            Consumer consumer = (Consumer) queryPlan;
            inBuffer.put(consumer.getOperatorID(), new LinkedBlockingQueue<ExchangeMessage>());
